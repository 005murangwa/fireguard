 ...
 * =============================================================================
 * WHAT: Operations exposed only to other microservices (cron, inspection,
...
export async function getExtinguisherByCode(code: string): Promise<FireExtinguisherResponse> {
...
export async function listExpiringWithinDays(days: number): Promise<FireExtinguisherResponse[]> {
  const now = new Date();
...
/**
 * POST /internal/from-order — register units after admin approves a client
 * purchase order. Self-contained (no dependency on the public service layer).
 */
export async function createExtinguishersFromOrder(input: {
  ...
  const manufacturingDate = parseDate(new Date().toISOString().slice(0, 10));
  ...
  for (const item of input.items) {
    ...
    for (let i = 0; i < item.quantity; i++) {
      const code = generateExtinguisherCode();
      ...
      created.push(formatExtinguisher(record));
    }
  }
